...
/*
  ...
  For more details see http://crosscloud.org/0.1.0/spec/js (TODO)
...
(function(exports){
	...
	exports.PodClient = function PodClient(options){
		...
		window.addEventListener("message", function(event) {
			...
			if (that.pinger) {
				clearInterval(that.pinger)
				that.pinger = null;
				console.log('the iframe is alive; we can stop pinging!');
			}
	...
	pod._buildiframe = function(options) {
		...
		// Now we need to wait until the iframe is loaded and
